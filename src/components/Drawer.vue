...
const createOrder = async () => {
  try {
    ...
    const { data } = await axios.post('https://9e87f3738f6d8a9a.mokky.dev/orders', {
      items: cart.value,
    ...
    <div>
        ...
        <div class="bg-white w-96 h-full fixed right-0 top-0 z-20 p-8 flex flex-col">
            ...
            <div v-if="!totalPrice || orderId" class="flex h-full items-center">
                <InfoBlock v-if="!totalPrice && !orderId"  title="Корзина пустая" description="Добавьте хотя бы одну пару кроссовок, чтобы оформить заказ." image-url="package-icon.png"/>
            ...
            <div v-else class="flex flex-col flex-between h-full">
                <CartItemList/>   
